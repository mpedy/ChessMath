...
P_MANIFEST = json.loads(
    open("static/dist/pages/pages_manifest.json", "r", encoding="utf-8").read())
...
if REMOTE_DEBUG_WEINRE == 1:
    ...
    console.print(Panel.fit(body, title="WEINRE Module ACTIVATED",
...
async def mainRoute(request):
    ...
    try:
        ...
        if uuid == "Animatore":
            # Console di gioco dal punto di vista dell'animatore
        ...
        return templates.TemplateResponse('index.html', {"request": request, "nome": opt.getNameFromUUID(uuid), "codice": opt.codice, "manifest": manifest, "debug_script": REMOTE_DEBUG_WEINRE})
...
async def notifyAllWSExceptAnimatore(message):
    global opt
    global websockets
    for ws in websockets:
...
async def setPage(request):
    ...
    if code == opt.setPageCode:
        # Autorizzato a cambiare pagina
        new_page = request.path_params['page']
        if new_page in range(0, len(P_MANIFEST[opt.percorso])):
            opt.page = new_page
            opt.Answered = {}
            opt.Answered[opt.page] = []
        res = [
            P_MANIFEST[opt.percorso][opt.page-1] if opt.page-1 in range(0, len(P_MANIFEST[opt.percorso])) else None,
            P_MANIFEST[opt.percorso][opt.page] if opt.page in range(0, len(P_MANIFEST[opt.percorso])) else None,
            P_MANIFEST[opt.percorso][opt.page+1] if opt.page+1 in range(0, len(P_MANIFEST[opt.percorso])) else None
        ]
        await notifyAllWS()
...
async def setPath(request):
    global opt
    opt.changePath(request.path_params['path'])
    await notifyAllWSExceptAnimatore(json.dumps({"path": str(opt.percorso)}))
    res = [
            P_MANIFEST[opt.percorso][opt.page-1] if opt.page-1 in range(0, len(P_MANIFEST[opt.percorso])) else None,
            P_MANIFEST[opt.percorso][opt.page] if opt.page in range(0, len(P_MANIFEST[opt.percorso])) else None,
            P_MANIFEST[opt.percorso][opt.page+1] if opt.page+1 in range(0, len(P_MANIFEST[opt.percorso])) else None
...
async def getquiz(request):
    global opt
    quizid = int(request.query_params.get("quizid", 1))
    try:
    ...
    try:
        ...
        if indiceDelNomeDaAggiungere >= 0:
            ...
            while indiceDelNomeDaAggiungere >= 0:
                indiceDelNomeDaAggiungere = opt.allNames.index(
                    nomeDaAggiungere+"-"+str(numDiNomiUguali))
                numDiNomiUguali += 1
    except Exception:
...
async def addPoints(request):
    ...
    if nome != "Animatore":
        ...
        opt.Classifica_ordered = sorted(
            opt.Classifica.items(), key=lambda x: x[1], reverse=True)
            ...
            for ws in websockets:
                if ws["cod"] == opt.codice and ws["name"] == "Animatore-console":
                    ...
                    try:
                        ...
                        # Tolgo l'animatore, una per l'iframe e un'altra per la console
                        l_tot = len(websockets) - 2
                        if l_tot != 0:
...
async def getClassifica(request):
    ...
    for i in range(0, tillPosition):
        result[i+1] = opt.Classifica_ordered[i] if i in range(
            0, len(opt.Classifica_ordered)) else None
...
async def refresh(request):
    global manifest
    manifest = json.load(
        open("static/dist/manifest.json", "r", encoding="utf-8"))
    return PlainTextResponse("ok")
...
async def polling(request):
    ...
    if codice in opt.UUID_NAME or codice == "Animatore":
        return JSONResponse({"pagina": opt.page, "path": opt.percorso})
    return PlainTextResponse("Codice non valido", status_code=401)
...
class MyWebSocket(WebSocketEndpoint):
    ...
    async def on_connect(self, websocket: WebSocket) -> None:
        ...
        if websocket.path_params["cod"] == opt.codice:
            websockets.append(
                {"cod": websocket.path_params["cod"], "name": websocket.path_params["name"], "ws": websocket})
    ...
    async def on_receive(self, websocket: WebSocket, data: typing.Any) -> None:
        ...
        if data == "get_answered":
            ...
            try:
                numerator = len(opt.Answered[opt.page])
                # Tolgo l'animatore, una websocket per l'iframe e un'altra per la console
                l_tot = len(websockets) - 2
                if l_tot != 0:
        ...
        await websocket.send_text(json.dumps({"pagina": opt.page, "path": opt.percorso}))
...
routes = [
    ...
    Route("/setpath_{path:str}", setPath),  # ok
...
app = Starlette(routes=routes,
                ...
                exception_handlers={405: onerror}
                )
